...
heading =Label(root,text='Moyale Linear Online Booking Parcel Portal', fg='white',
               bg='#161C25',font=('Microsoft Yahei UI Light',15,'bold'))
heading.place(x=205,y=5)
sub_heading =Label(root,text='Passenger should provide original national ID/ Passport', fg='white',
               bg='#161C25',font=('Microsoft Yahei UI Light',15,'bold'))
sub_heading.place(x=150,y=50)
def go_to_admin():
    import Admin
    root.destroy()

    
Button(root,width=15,pady=1,text='Admin Page', bg='#57a1f8',fg='white',border=0,font=('Microsoft Yahei UI Light',10,'bold'),command=go_to_admin).place(x=350,y=100)
#---  
tree=ttk.Treeview(root,columns=("PARCEL","DEPARTURE","ARRIVAL","FULL_NAME","ID_NUMBER","PHONE_NUMBER","EMAIL","ID_IMAGE"), show="headings",height=450)
tree.heading("PARCEL",text="PARCEL")
